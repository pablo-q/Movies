<?php
    ...
    if ( null==$id ) {
        header("Location: showDirectores.php");
    }
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            ...
            $sql = "UPDATE directores set nombre = ?, apellido = ?, pais = ? WHERE id = ?";
            $q = $pdo->prepare($sql);
        ...
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = 'select directores.id AS id, directores.nombre AS nombre, apellido, paises.nombre AS pais, paises.id as paisId FROM directores INNER JOIN paises ON directores.pais = paises.id WHERE directores.id = ?';
        $q = $pdo->prepare($sql);
        $q->execute(array($id));
...
<body>
    <div class="container">
                ...
                <div class="span10 offset1">
                    ...
                    <form class="form-horizontal" action="updateDirector.php?id=<?php echo $id?>" method="post">
                      ...
                      <div class="form-actions">
                          ...
                          <a class="btn btn-default" href="showDirectores.php">Volver</a>
                        </div>
                    </form>
                </div>
